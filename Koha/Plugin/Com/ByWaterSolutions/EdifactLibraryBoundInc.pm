package Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc;
...
our $metadata = {
    name            => 'Edifact - Library Bound Inc',
    author          => 'Kyle M Hall',
    description     => 'Edifact plugin for Library Bound Inc',
...
sub edifact {
    ...
    require Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact;

    my $edifact = Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact->new($args);
    return $edifact;
}
...
sub edifact_order {
    ...
    require Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact::Order;

    $args->{params}->{plugin} = $self;
    my $edifact_order = Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact::Order->new( $args->{params} );
...
sub edifact_transport {
    ...
    require Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact::Transport;

    $args->{params}->{plugin} = $self;

    my $edifact_transport = Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact::Transport->new( $args->{vendor_edi_account_id}, $self );
...
sub configure {
    ...
    unless ( $cgi->param('save') ) {
        ...
        $template->param(
            ...
            order_contact_name                             => $self->retrieve_data('order_contact_name'),
            order_contact_email                            => $self->retrieve_data('order_contact_email'),
            send_shipto_address                            => $self->retrieve_data('send_shipto_address'),
            shipto_address_qualifier                       => $self->retrieve_data('shipto_address_qualifier'),
            send_billto_address                            => $self->retrieve_data('send_billto_address'),
            billto_address_qualifier                       => $self->retrieve_data('billto_address_qualifier'),
            ...
            lin_use_marc_field                             => $self->retrieve_data('lin_use_marc_field'),
            lin_use_marc_field_qualifier                   => $self->retrieve_data('lin_use_marc_field_qualifier'),
            pia_marc_fields                                => $self->retrieve_data('pia_marc_fields'),
        ...
        my $new_settings = {
            ...
            order_contact_name                             => $cgi->param('order_contact_name')  || q{},
            order_contact_email                            => $cgi->param('order_contact_email') || q{},
            send_shipto_address                            => $cgi->param('send_shipto_address')                 ? "1" : "0",
            shipto_address_qualifier                       => $cgi->param('shipto_address_qualifier') || 'DP',
            send_billto_address                            => $cgi->param('send_billto_address')                 ? "1" : "0",
            billto_address_qualifier                       => $cgi->param('billto_address_qualifier') || 'IV',
            ...
            lin_use_marc_field                             => $cgi->param('lin_use_marc_field')           || q{},
            lin_use_marc_field_qualifier                   => $cgi->param('lin_use_marc_field_qualifier') || q{},
            pia_marc_fields                                => $cgi->param('pia_marc_fields')              || q{},
