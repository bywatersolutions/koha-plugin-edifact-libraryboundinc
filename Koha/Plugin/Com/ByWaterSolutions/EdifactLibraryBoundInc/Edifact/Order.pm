package Koha::Plugin::Com::ByWaterSolutions::EdifactLibraryBoundInc::Edifact::Order;
...
sub order_line {
    ...
    if ($lin_use_item_field) {
        ...
        if ($aqorder_item) {
            ...
            # The aqorders_items row is not removed when an item is deleted,
            # so the item may no longer exist
            if ($item) {
